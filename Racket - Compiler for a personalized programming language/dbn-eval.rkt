...
(define (eval-statement env statement [slow? #f])
  ...
  (match statement
    ...
    [(line-expr x y x1 y1) (draw-line (eval-expr env x) (eval-expr env y) (eval-expr env x1) (eval-expr env y1)) env]
    ...
    [(assignment-expr (var-expr name) value)
     (let ([refer (apply-env env name)])
       (if refer
           (begin (setref! refer (eval-expr env value)) env)
    ...
    [(command-fun sym params body)
     (let ([func (closure sym params body env)])
       (extend-env env sym func))]
    ...
    [(apply-expr sym exprs)
     (let ([refer (apply-env env sym)])
       (if refer (letrec ([cls (deref refer)]
                          [params (closure-params cls)]
                          [body   (closure-body cls)]
                          [oldenv (closure-env cls)]
                          [newenv (map (λ (p vexprs) (cons p (memref p (eval-expr env vexprs)))) params exprs)])
                  (eval-statements (append newenv env) body slow?)
                   env
                   )
    ...
    [(apply-expr sym exprs)
     (let ([refer (apply-env env sym)])
       (if refer (letrec ([cls      (deref refer)]
                          ...
                          [oldenv   (closure-env cls)]
                          [newenv   (map (λ (p vexprs) (cons p (memref p (eval-expr env vexprs)))) params exprs)]
                          [evalenv  (append newenv env)])
                          (if (null (cdr exprs))
                              (eval-expr evalenv (car exprs))
                              (begin
                                (eval-expr evalenv (car exprs))
                                (apply-expr sym (cdr exprs))
                                )
                              ))
           (error "cannot find" sym)))] 
    ))
